...
class _CallScreenState extends State<VideoCallScreen> {
  ...
  _setupPeerConnection() async {
    ...
    if (widget.offer != null) {
      ...
      //   'databases.$databaseId.collections.$iceCandidateCollection.documents'
      // ]);
      ...
      subscription.stream.listen((response) async {
        ...
        if (response.events
          ...
          for (RTCIceCandidate candidate in rtcIceCadidates) {
            ...
            //   databaseId: databaseId,
            //   collectionId: iceCandidateCollection,
            //   documentId: uniqueId,
            //   data: {
            //     'id': candidate.sdpMid,
            //     'label': candidate.sdpMLineIndex,
  ...
  void _setResolution(Resolution resolution) {
    ...
    _localStream!.getVideoTracks().forEach((track) {
      track.applyConstraints({
        'audio': true,
        'video': {
          'width': resolution == Resolution.HD ? '1280' : '640',
          'height': resolution == Resolution.HD ? '720' : '480',
          'frameRate': '30',
        },
      });
    });
  ...
  void toggleResolution() {
    setState(() {
      _isHD = !_isHD;
      _setResolution(_isHD ? Resolution.HD : Resolution.SD);
    });
  }
